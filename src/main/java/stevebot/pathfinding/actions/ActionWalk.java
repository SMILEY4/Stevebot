...
public class ActionWalk extends Action {
	...
	public ActionWalk(Node from, BlockPos to, BlockPos[] touching) {
		this.from = from;
		this.to = Node.get(to);
		this.cost = Action.COST_WALKING * (Action.isDiagonal(from.pos, to) ? 1.414 : 1) * (touching == null ? 1 : 1.8);
...
//		Stevebot.RENDERER.addObject(new Marker(controller.getPlayerPosition(), this, Color.CYAN));
